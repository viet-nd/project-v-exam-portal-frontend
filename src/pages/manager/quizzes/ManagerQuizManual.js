...
const ManagerQuizManual = () => {
  ...
  const startQuizHandler = (quizTitle, quizId) => {
    navigate(`/managerStartQuiz/?quizId=${quizId}&quizTitle=${quizTitle}`);
  };
  ...
  useEffect(() => {
    if (quizzes.length == 0) {
      fetchQuizzByQuizId(dispatch, quizId, token).then((data) => {
...
export default ManagerQuizManual;
